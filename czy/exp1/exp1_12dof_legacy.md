# 实验记录（12DOF 旧项目 X1_29_re0 · 历史存档）

> 本文件为 12DOF 项目（X1_29_re0）exp0~exp1.4 完整历史，仅作 29DOF 项目的先验参考。
> 29DOF 项目（X1_29_amp）实验记录见 [exp1.md](exp1.md)（从 exp0 重新编号）。
...
| exp2 | 2026-09-02 | 29DOF 全身控制改造：env 腿部按名索引 + config 29 维（obs 98/action 29/priv 141）+ 29DOF PM URDF + 上半身默认位姿锁定，从零 L4 训练至 5800 轮额度耗尽；回放摔倒（min_h 0.092m）+ 严重过冲（0.4 段 286%）+ 停不住，站立段完美 | ❌未达标（已测试） | TASK_20260902_185(停)→186 | limxmtcm6wjlso8ce4@emalupe.com（账号3，已耗尽） | model_5800.pt |
...
## 实验 exp2：29DOF 全身控制改造（阶段2 首次尝试，修改编号重置）

### 1. 上一实验结果与教训

> 数据：exp1.4 §7 + 本轮改造验证（2026-09-02）
> - exp1.4 遗留：偏航漂移回退（0.4 段 14.42°）、侧漂回退（+1.202m）、跟踪 70.6%/69.3%——12DOF 侧尚未收敛
> - **跨模块需求**：项目目标为 29DOF 全身（X1_29_amp），amp CSV 参考运动已备（29 关节 30Hz 行走数据），当前 12DOF 代码无法承载
> - **本轮改造验证**（改 config 前后分步执行）：
...
>   - FK 镜像：默认位姿位置残差 0.00mm、姿态 0.00°（右踝 pitch=+0.21 判别严格：错误符号显 24.06°，与 exp0 记录吻合）
>
> **核心教训**：
> - Isaac Gym dof 顺序为字母序 DFS（左腿0-5/腰6-8/左臂9-15/右臂16-22/右腿23-28），**非 URDF 文档顺序**——armature 逐关节编号曾按文档序写错，被启动打印的 [DOF] 表当场纠正
> - F1 环境 humanoid 包原 editable 指向旧仓库 X1_29_re0，`python scripts/train.py`（脚本目录优先）会绕过 cwd 使用旧代码，冒烟前必须 `pip show` 确认安装路径
...
| `default_joint_angles` 上半身 17 项 | 无 | lumbar(0/0/0.03)、肩(0.03/-0.06/0.18)、肘(0.34/0)、腕(0) | amp CSV 均值左右对称化（exp1.1 不对称教训） |
...
**理由**：29DOF URDF 腿部与 12DOF PM 同源（同轴系/限位/质量 36.462kg），腿部动力学配置直接继承；上半身无辨识数据，按"不猜中心、宽覆盖"原则（exp1.4 踝策略迁移）。
...
| 启动命令 | `gm-run X1_29_amp/humanoid/scripts/train.py --task=x1_dh_stand --run_name=exp2_29dof_baseline --headless --max_iterations=6000` |
...
| 指标 | 12DOF 参考（exp0 本机首轮） | 本轮目标 | 异常信号 |
| --- | --- | --- | --- |
| Mean reward | 147.8 | ≥ 120 | < 80 |
| Mean episode length | 2210/2400 | ≥ 2100 | < 1500 |
| 回放跟踪 0.4/0.6 | 70.6%/69.3%（exp1.4） | ≥ 65% | < 55% |
...
> 三件套归档（lab-notebook §8）：`czy/data/exp2/` = model_5800.pt + play_output.mp4（36.6MB，40s，1:1 速度）+ isaac_diag.csv（2000 行 × 15 列，与视频同 run）
